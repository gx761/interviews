...
var myAtoi = function(s) {
  ...
  if(inx <s.length && s.charAt(inx) === '+') {
    inx++;
  } else if(inx <s.length && s.charAt(inx) === '-') {
    inx++;
    flag = -1;
  }

  if(s.charAt(inx) > '9' || s.charAt(inx) <'0') {
    return 0;
  }

  while(inx< s.length&& s.charAt(inx) <= '9' && s.charAt(inx) >='0') {
    const digit = parseInt(s.charAt(inx)) * flag;
    res = res * 10 + digit;
    inx++;
  }
