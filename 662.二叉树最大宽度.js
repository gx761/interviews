...
var widthOfBinaryTree = function(root) {
  ...
  while(stack.length) {
    ...
    for(let i=0; i<len;i++) {
      ...
      if(node.left) {
        stack.push([node.left, (2 * level) % Number.MAX_SAFE_INTEGER ]);
      }
  
      if(node.right) {
        stack.push([node.right, (2 * level+1) % Number.MAX_SAFE_INTEGER]);
      }
    }

    if(stack.length) {
      max = Math.max(max, stack[stack.length-1][1] - stack[0][1] +1) ;
    }
  }
  return max;
